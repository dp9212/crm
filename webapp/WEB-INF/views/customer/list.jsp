  ...
  series: [{
	...
	//---------------添加访问数据字段数据---------------------	
		...
		customerForm.form({
			...
			// 在表单提交前，做事情
			onSubmit : function() {
				alert(customerForm.form("validate"));
		...
		var cmdUtils = {
				create : function() {
					//修改对话框标题
					customerDialog.dialog("setTitle", "部门添加面板")
					// 清空对话框
				...
				edit : function() {
					//修改对话框标题
					customerDialog.dialog("setTitle", "部门修改面板")
					// 获取选中行数据
					...
					if (rowData.source) {
						rowData["source.id"] = rowData.source.id;
					}
				...
				del : function() {
					...
					$.messager.confirm("温馨提示","是否确认删除【" + rowData.name + "】部门??",	function(yes) {
						if (yes) {
							...
							var id = rowData.id;
							// 发送AJAX请求，修改ID对应数据状态
							$.get("/customer/delete?id="+ id,function(data) {
								if (data.success) {
									$.messager.alert("温馨提示",data.msg,	"info",function() {
											customerDataGrid.datagrid("reload");
									});
				...
				search : function() {//简单搜索：查询条件必须少
					customerDataGrid.datagrid("load",$("#searchForm")
							.serializeJSON1());
				},
				//点击弹出高级查询框
				highSearch: function() {
				...
				highSearchForm:function() {
					customerDataGrid.datagrid("load",$("#highSearchForm")
							.serializeJSON1());
				...
				pass:function() {
					var rowData = customerDataGrid.datagrid("getSelected");
					if(rowData.state==1){
						$.messager.alert("温馨提示","已放入资源池,无法移交");
						return;
					}
					if(rowData.state==-1){
				...
				follow:function() {
					var rowData = customerDataGrid.datagrid("getSelected");
					if(rowData.state==1){
						$.messager.alert("温馨提示","已放入资源池,无法跟进");
						return;
					}
				...
				putsource:function() {
					...
					$.get("/customer/changeState?id="+ id,function(data) {
				...
				dlgClose:function() {
					...
					highSearchForm.attr("action","\/customer/dcExcel");
					highSearchForm.submit();
					highSearchForm.attr("action","\/customer/save");
				}
...
<body>
	<table id="customerDataGrid" class="easyui-datagrid" title="Basic DataGrid"
		fit="true" style="width: 700px; height: 250px" toolbar="#toolbar"
		rownumbers="true" pagination="true" pageList="[10,20,50]"
		fitColumns="true"
		data-options="singleSelect:true,url:'/customer/json',method:'get'">
		<thead>
			<tr>
				...
				<th data-options="field:'state',width:10" formatter="stateFormatter">状态</th>
	...
	<div id="toolbar">
		...
		<a data-cmd="coming" 
		 ...
		 <a href="javascript:void(0)" class="easyui-linkbutton" onclick="$('#dlg').dialog('open')">薪水饼图</a>
			...
			<a href="/customer/dcExcel">导出</a>
		</div>
	</div>

	<div id="customerDialog" class="easyui-dialog" title="保存用户"
		data-options="iconCls:'icon-save',closed:true"
		style="width: 300px; height: auto; padding: 20px" buttons="#btns">
		<form id="customerForm" method="post" action="/customer/save">
	...
	<div id="btns">
		...
		<form id="highSearchForm" method="post" action="/customer/save">
			<input type="hidden" name="id"/>
			<table>
				<tr>
				<td>名称:</td> 
				...
				<tr>
				<td>年龄大于:</td> 
				<td> <input class="easyui-slider" name="age" style="width:160px,margin:15px" data-options="showTip:true"></td>
				 </tr>
				<tr><td>登入人:</td><td>
	...
	<div id="transferDialog" class="easyui-dialog" title="移交客户"
		...
		<form id="transferForm" method="post" action="/customer/transfer">
			<input type="hidden" name="id"/>
			<table>
				<tr>
				<td>客户姓名:</td> 
	...
	<div id="customerTraceHistorybtns">
	    ...
	    <div id="dlg" class="easyui-dialog" title="Basic Dialog" data-options="iconCls:'icon-save',closed:true" style="width:600px;height:500px;padding:10px"
	    buttons="#dlgBtns"
	    >
       		<div id="container" style="height: 400px"></div>
    	</div>
	<div id="dlgBtns">
		<a href="javascript:void(0)" class="easyui-linkbutton c3" data-cmd="dlgClose">关闭</a> 
	</div>
